...
impl<'a> Widget for TreeWidget<'a> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        for session in self.state.sessions.values() {
            ...
            if is_selected { style = style.add_modifier(Modifier::BOLD); }

            let line = Line::from(vec![
                Span::styled(format!("{} ", icon), style.fg(Color::Yellow)),
                Span::styled(session.name.clone(), style),
                Span::styled(format!(" ({})", session.windows.len()), style.fg(Color::Gray)),
            ]);
            ...
            if session.expanded {
                for window_id in &session.windows {
                    ...
                    if let Some(window) = self.state.windows.get(window_id) {
                        let is_win_selected = Some(&window.id) == self.state.focus.selected_id.as_ref();
                        let win_bg_color = if is_win_selected { Color::DarkGray } else { Color::Reset };
                        ...
                        let win_icon = if window.panes.is_empty() { " " } else if window.expanded { "▼" } else { "▶" };
                        let line = Line::from(vec![
                            Span::styled(format!("  {} ", win_icon), Style::default().bg(win_bg_color).fg(Color::Yellow)),
                        ...
                        if window.expanded {
                            ...
                            for (idx, pane_id) in window.panes.iter().enumerate() {
                                ...
                                if let Some(pane) = self.state.panes.get(pane_id) {
                                    let is_pane_selected = Some(&pane.id) == self.state.focus.selected_id.as_ref();
                                    let pane_bg_color = if is_pane_selected { Color::DarkGray } else { Color::Reset };
